      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        ...
        >
          <div className="relative aspect-video">
            ...
            <button
              onClick={prevImage}
              className="absolute left-4 top-1/2 transform -translate-y-1/2 z-20 bg-white/80 hover:bg-white text-forest-700 p-2 rounded-full transition-all duration-300"
            ...
            <button
              onClick={nextImage}
              className="absolute right-4 top-1/2 transform -translate-y-1/2 z-20 bg-white/80 hover:bg-white text-forest-700 p-2 rounded-full transition-all duration-300"
              ...
              {farmImages.map((_, index) => (
                <button
                  ...
                  className={`w-3 h-3 rounded-full transition-all duration-300 ${
                    activeImage === index ? 'bg-forest-600 w-6' : 'bg-white/70 hover:bg-white'
                  }`}
        ...
        <div className="grid md:grid-cols-3 gap-8 mt-12">
          {[
            ...
            <div
              key={index}
              className={`bg-white rounded-lg p-6 shadow-soft transition-all duration-700 hover:shadow-md ${
                sectionVisible ? 'opacity-100 transform-none' : 'opacity-0 translate-y-10'
              }`}
              style={{ transitionDelay: `${600 + index * 100}ms` }}
            >
              <h3 className="text-xl font-bold text-forest-700 mb-2">{fact.title}</h3>
              <p className="text-earth-700">{fact.description}</p>
            </div>
          ))}
        </div>
        
        <div className={`mt-12 text-center transition-all duration-700 delay-900 ${
          sectionVisible ? 'opacity-100 transform-none' : 'opacity-0 translate-y-10'
        }`}>
          <a
            href="#visit-us"
            className="inline-block px-8 py-3 bg-forest-600 text-white font-medium rounded-full transition-all duration-300 hover:bg-forest-700 transform hover:scale-105 shadow-md hover:shadow-lg"
          >
            Book a Farm Tour
          </a>
        </div>
