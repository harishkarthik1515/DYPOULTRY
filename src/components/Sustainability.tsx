      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
          {ethicalPractices.map((practice, index) => (
            <div
              key={practice.id}
              className={`bg-white rounded-xl p-8 shadow-lg transition-all duration-700 hover:shadow-xl hover:translate-y-[-5px] ${
                sectionVisible ? 'opacity-100 transform-none' : 'opacity-0 translate-y-10'
              }`}
              style={{ transitionDelay: `${300 + index * 100}ms` }}
            >
              <div className="mb-6 w-16 h-16 rounded-full bg-primary-200 flex items-center justify-center">
                {getIcon(practice.icon)}
              </div>
              <h3 className="text-xl font-bold text-primary-700 mb-4">{practice.title}</h3>
              <p className="text-primary-700 leading-relaxed">{practice.description}</p>
            </div>
          ))}
        </div>

        <div 
          className={`bg-primary-600 rounded-2xl p-12 shadow-lg text-white transition-all duration-700 delay-600 ${
            sectionVisible ? 'opacity-100 transform-none' : 'opacity-0 translate-y-10'
          }`}
        >
          <div className="lg:flex items-center justify-between">
            <div className="lg:w-2/3 mb-6 lg:mb-0">
              <h3 className="font-serif text-2xl lg:text-3xl font-bold mb-4">Our Sustainability Pledge</h3>
